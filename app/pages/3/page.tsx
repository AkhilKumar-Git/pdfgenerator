...
export default function BloodParameters() {
    ...
    <div className="blood-parameters-container w-[210mm] min-h-[297mm] bg-white px-[48px] pt-[72px]">
      <Card className="blood-parameters-card">
        ...
        <CardContent>
          ...
          {dynamicData.Tests.map((test, index) => (
            <div key={index}>
              <h3 className="text-lg font-semibold">{test.page_type}</h3>
              {test.complete_blood_picture &&
                test.complete_blood_picture.map((param, paramIndex) => (
                  <ParameterRow key={paramIndex} param={param} />
                ))}
              {/* Add similar mapping for other test types if needed */}
            </div>
          ))}
